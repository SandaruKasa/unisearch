...
def valfromcp(n, cp=None):
    ...
    if cp:
        ...
        cps = ( struct.pack(chr(0x42),i) for i in cps ) # this works in both python3 and python2, unlike bytes([i])
        cps = b''.join(cps)
        try:
            cps = cps.decode(cp)
...
def print_characters(clist, maxcount, format_string, query_wikipedia=0, query_wiktionary=0):
    ...
    for c in clist:
        ...
        if (not name) or name.startswith('<'):
            unicode1name = properties['unicode1name']
            if unicode1name and not unicode1name.startswith('<'):
                name = unicode1name
            else:
                internal_name = unicodedata.name(c, '')
                if internal_name:
                    name = internal_name
        if not name:
        ...
        if 0xd800 <= ordc <= 0xdfff: # surrogate
            ...
            utf8 = ' '.join([("%02x" % x) for x in c.encode('utf-8')])
            utf16be = ''.join([("%02x" % x) for x in c.encode('utf-16be')])
        decimal = "&#%s;" % ordc
        octal = "\\0%o" % ordc
        ...
        if addcharset:
            try:
                in_additional_charset = ' '.join([("%02x" % x) for x in c.encode(addcharset)] )
            except UnicodeError:
